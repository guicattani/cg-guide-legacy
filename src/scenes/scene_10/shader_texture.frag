...
void main()
{
    ...
    if(!isObject) {
      ...
      if(texture_projection != CUBE_MAP)
        Kd0 = vec4(texture(diffuseTexture, vec2(U,V)).rgb, texture_projection_transparency);
      else if (cube_map_index == 0)
        Kd0 = vec4(texture(cubemapLeftTexture, vec2(U,V)).rgb, texture_projection_transparency);
      else if (cube_map_index == 1)
        Kd0 = vec4(texture(cubemapRightTexture, vec2(U,V)).rgb, texture_projection_transparency);
      else if (cube_map_index == 2)
        Kd0 = vec4(texture(cubemapTopTexture, vec2(U,V)).rgb, texture_projection_transparency);
      else if (cube_map_index == 3)
        Kd0 = vec4(texture(cubemapBottomTexture, vec2(U,V)).rgb, texture_projection_transparency);
      else if (cube_map_index == 4)
        Kd0 = vec4(texture(cubemapFrontTexture, vec2(U,V)).rgb, texture_projection_transparency);
      else
