...
// ADD-ONS
#include <map>
#pragma region[rgba(100, 0, 0, 0.1)] ADDONS
struct PlotVarData
{
  ImGuiID ID;
  ImVector<float> Data;
...
void ImGui::PlotVarFlushOldEntries()
{
  ...
  for (PlotVarsMap::iterator it = g_PlotVarsMap.begin(); it != g_PlotVarsMap.end();)
  {
    ...
    if (pvd.LastFrame < current_frame - ImMax(400, (int)pvd.Data.size()))
...
#undef NAV_MAP_KEY
  ...
  if (g.NavMoveRequestForward == ImGuiNavForward_None)
  {
    ...
    {
      if (!IsActiveIdUsingNavDir(ImGuiDir_Left) && IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadLeft, ImGuiNavInput_KeyLeft_, ImGuiInputReadMode_Repeat))
      {
        g.NavMoveDir = ImGuiDir_Left;
      }
      if (!IsActiveIdUsingNavDir(ImGuiDir_Right) && IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadRight, ImGuiNavInput_KeyRight_, ImGuiInputReadMode_Repeat))
      {
        g.NavMoveDir = ImGuiDir_Right;
      }
      if (!IsActiveIdUsingNavDir(ImGuiDir_Up) && IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadUp, ImGuiNavInput_KeyUp_, ImGuiInputReadMode_Repeat))
      {
        g.NavMoveDir = ImGuiDir_Up;
      }
      if (!IsActiveIdUsingNavDir(ImGuiDir_Down) && IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadDown, ImGuiNavInput_KeyDown_, ImGuiInputReadMode_Repeat))
      {
